...
with DAG(dag_id='scst_atrocities',
    ...
    def extract_data(offset):
        ...
        endpoint=f'//nic-data?url=http://164.100.251.114/scst/scst/atrocities/tbl-scst-case-details&count={count}&offset={offset}'
    ...
    @task()
    def load_data(df, table_name,offset_table):
            if df.empty:
            ...
            try:
                ...
                query = text(f"update bipard_staging.table_status set offset_of_table=offset_of_table+{int(len(df))} where table_name='{offset_table}';")
                print(query)
    ...
    offset=get_offset('crime_against_sc_st') #change table name here 
    t_data=extract_data(offset=offset)
    trans_data=transform_data(t_data)
    load_data(trans_data,'crime_against_sc_st_1','crime_against_sc_st')
