...
with DAG(dag_id='Cooperative_Procurement_PADDY',
    ...
    def extract_data(offset):
        ...
        endpoint=f'/nic-data?url=http://164.100.251.114/coop/coop/procurement/paddy&count={count}&offset={offset}'
    ...
    offset=get_offset('Cooperative_Procurement_PADDY') #change table name here 
    t_data=extract_data(offset=offset)
    trans_data=transform_data(t_data)
    load_data(trans_data,'Cooperative_Procurement_PADDY')
